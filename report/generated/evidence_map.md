...
| Default sample rate is 100 kHz and target maximum is 1 MHz. | `src/firmware/board_config.h`; `src/firmware_v2/board_config.h` | `LA_DEFAULT_SAMPLE_RATE_HZ` and `LA_MAX_SAMPLE_RATE_HZ_TARGET` define these values. |
...
| An Arduino UNO based test-signal source is available. | `tools/arduino_signal_generator/src/arduino_signal_generator.ino` | The sketch generates pulse, UART and I2C test signals. |
| Firmware v2 DMA/benchmark performance has not been measured on board. | `src/firmware_v2/README.md`; `src/firmware_v2/main.cpp` | README notes that board measurement is still needed; firmware prints not-measured status. |
