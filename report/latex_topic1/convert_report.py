...
\section{Giao thức UART}\label{sec:uart-theory}
...
\begin{tikzpicture}[font=\scriptsize, >=Latex]
  % --- geometry ---
  \def\bw{1.08}   % bit cell width
  \def\bh{0.65}   % bit cell height
  \def\wy{-2.3}   % waveform baseline y
  \def\wh{0.55}   % waveform high amplitude

  % ---- FRAME CELLS ----
  % START (filled)
  \fill[gray!55] (0,0) rectangle (\bw,\bh);
  \draw[thick] (0,0) rectangle (\bw,\bh);
  \node[font=\scriptsize\bfseries] at ({0.5*\bw},{0.5*\bh}) {START};

  % D0..D7
  \foreach \i in {0,...,7} {
    \draw[thick] ({(\i+1)*\bw},0) rectangle ({(\i+2)*\bw},\bh);
    \node at ({(\i+1.5)*\bw},{0.5*\bh}) {D\i};
  }

  % PB (dashed border = optional)
  \draw[thick,dashed] ({9*\bw},0) rectangle ({10*\bw},\bh);
  \node at ({9.5*\bw},{0.5*\bh}) {PB};

  % STOP (filled)
  \fill[gray!55] ({10*\bw},0) rectangle ({11*\bw},\bh);
  \draw[thick] ({10*\bw},0) rectangle ({11*\bw},\bh);
  \node[font=\scriptsize\bfseries] at ({10.5*\bw},{0.5*\bh}) {STOP};

  % ---- LABELS ABOVE ----
  \node[above,align=center] at ({0.5*\bw},\bh) {Start bit\\logic 0};
  % bracket over D0-D7
  \draw ({1.05*\bw},{\bh+0.55}) -- ({1.05*\bw},{\bh+0.35}) --
        ({8.95*\bw},{\bh+0.35}) -- ({8.95*\bw},{\bh+0.55});
  \node[above] at ({5*\bw},{\bh+0.35}) {Word data};
  \node[above,align=center] at ({9.5*\bw},\bh) {Parity bit\\(optional)};
  \node[above,align=center] at ({10.5*\bw},\bh) {Stop bit\\logic 1};

  % ---- WAVEFORM ----
  % idle high before START
  \draw[thick] ({-0.6},{\wy+\wh}) -- (0,{\wy+\wh});
  % START: falling edge, hold low for 1 bit
  \draw[thick] (0,{\wy+\wh}) -- (0,\wy) -- (\bw,\wy);
  % D0=0 low
  \draw[thick] (\bw,\wy) -- ({2*\bw},\wy);
  % D1=1 rising
  \draw[thick] ({2*\bw},\wy) -- ({2*\bw},{\wy+\wh}) -- ({3*\bw},{\wy+\wh});
  % D2=1 high
  \draw[thick] ({3*\bw},{\wy+\wh}) -- ({4*\bw},{\wy+\wh});
  % D3=0 falling
  \draw[thick] ({4*\bw},{\wy+\wh}) -- ({4*\bw},\wy) -- ({5*\bw},\wy);
  % D4=1 rising
  \draw[thick] ({5*\bw},\wy) -- ({5*\bw},{\wy+\wh}) -- ({6*\bw},{\wy+\wh});
  % D5=0 falling
  \draw[thick] ({6*\bw},{\wy+\wh}) -- ({6*\bw},\wy) -- ({7*\bw},\wy);
  % D6=1 rising
  \draw[thick] ({7*\bw},\wy) -- ({7*\bw},{\wy+\wh}) -- ({8*\bw},{\wy+\wh});
  % D7=1 high
  \draw[thick] ({8*\bw},{\wy+\wh}) -- ({9*\bw},{\wy+\wh});
  % PB=0 falling (example)
  \draw[thick] ({9*\bw},{\wy+\wh}) -- ({9*\bw},\wy) -- ({10*\bw},\wy);
  % STOP: rising, hold high, then idle
  \draw[thick] ({10*\bw},\wy) -- ({10*\bw},{\wy+\wh}) -- ({11.6*\bw},{\wy+\wh});

  % sampling tick marks at center of each bit
  \foreach \i in {0,...,10} {
    \pgfmathsetmacro{\cx}{(\i+0.5)*\bw}
    \draw ({(\i+0.5)*\bw},{\wy-0.12}) -- ({(\i+0.5)*\bw},{\wy+0.12});
  }

  % ---- LABELS BELOW WAVEFORM ----
  \node[below,align=center] at ({0.5*\bw},{\wy-0.18})
    {Start by detecting\\transition 1\,{\scriptsize$\rightarrow$}\,0};
  \draw[<->] ({\bw},{\wy-0.85}) -- ({9*\bw},{\wy-0.85});
  \node[below] at ({5*\bw},{\wy-0.85}) {Incoming data sampled at the bit-pulse center};
  \node[below,align=center] at ({10.5*\bw},{\wy-0.18}) {Sample\\stop bit};

  % vertical dashed guides connecting frame to waveform
  \foreach \x in {0,\bw,...,10.01} {
    \pgfmathsetmacro{\xx}{\x*\bw}
    % skip — just use natural positions
  }
\end{tikzpicture}
\caption{Cấu trúc khung UART 8N1 và thời điểm lấy mẫu}
