...
Generator mặc định khởi động ở `MODE BOTH`, phát lần lượt UART trên D2/CH0,
I2C trên D3-D4/CH1-CH2 và SPI trên D5-D8/CH3-CH6 trong một superframe 40 ms.
Mọi đường đều dùng open-drain an toàn; Timer1 đồng thời phát các sóng vuông
open-drain trên nhóm kênh phụ:
...
Arduino nhận lệnh USB serial `MODE BOTH`, `MODE GRAY`, `MODE UART`, `MODE I2C`, `MODE SPI`,
`STATUS` và `PING` ở 115200 baud. `MODE BOTH` dùng UART 2400 baud, I2C khoảng
20 kHz và SPI khoảng 20 kHz; `MODE UART` riêng chạy khoảng 57,04 kbaud và
`MODE I2C` riêng chạy
...
`MODE GRAY`. `MODE SPI` phát open-drain trên CH3--CH6 với ba cặp MOSI/MISO
`55/A5`, `A5/3C`, `5A/C3`; mức HIGH do pull-up 3,3 V của STM32 tạo ra.
